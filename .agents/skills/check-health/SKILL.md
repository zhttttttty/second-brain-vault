---
name: check-health
description: 定期审查 Obsidian 知识库健康状况，包括：矛盾观点检测、失效双向链接检查、孤立卡片识别、标签一致性检查。当用户说「检查知识库健康」、「检查链接」、「查孤立卡片」、「检查标签」、「审查观点矛盾」或类似表达时触发此技能。
...
3. 报告以下问题：
   - 未登记、大小写不统一或同义并存的标签；
   - `card`、`insight`、`project`、`status` 等本应由 `type`、`card_type` 或其他 Properties 表达的标签；
   - 超过 3 个主题标签、只使用宽泛父标签、超过三级的标签；
   - 人名、年份、作者、版本号等不应标签化的值。
4. 只给出迁移建议，不自动新增分类；稳定新主题应先更新 Schema，再修改笔记。
