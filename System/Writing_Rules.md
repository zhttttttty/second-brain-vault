...
单来源卡片用 `source` 字段即可。综合 3+ 来源的卡片，可在段落末尾使用命名脚注，并在脚注定义中放入可点击的来源笔记或外部链接：

```markdown
价值投资的核心是安全边际，即买入价格显著低于内在价值。[^graham]

但安全边际在高速成长股上需要结合不确定性重新估计。[^damodaran]

[^graham]: [[格雷厄姆-聪明的投资者]]，安全边际相关章节。
[^damodaran]: [Damodaran Online](https://pages.stern.nyu.edu/~adamodar/)，估值材料。
```

命名脚注应稳定、简短、可辨认；不要用无法点击且语义不明的 `^[来源]`。逐段标记只说明“这条主张来自哪里”，不自动证明来源可靠或结论正确。重要结论仍应检查原文、发布日期、证据等级和是否存在独立反例。
