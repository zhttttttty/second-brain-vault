...
- 单个概念、观点或模型 → `03_Knowledge/00_Cards/`
- 长期领域的知识地图与综合理解 → `03_Knowledge/01_Topics/`
- 有目标、期限或完成标准的学习 → `02_Projects/`
- 外部文章与原始资料 → `04_References/`
- 人可直接执行的方法 → `mentalmodel` 或 `checklist` 卡片
- 用来指导 AI 执行的完整流程 → `.agents/skills/`

Topic 通过链接连接这些内容，不在目录中复制保存它们。

`Knowledge_System_Workflow.canvas` 提供从输入、提炼到卡片和 Topic 组装的可视导航。Canvas 只做空间导航，不替代本目录的 Markdown 主题地图。
