...
5. **可调用**：行动型或判断型卡片可用 `use_when` 标记 1～3 个具体使用时机；它不是主题标签
...
本目录预置了 10 张示例卡片（`_EXAMPLE_` 前缀），展示不同类型的卡片长什么样，它们之间怎么互相引用。其中 3 张 `model` 卡片演示「1 小时构建知识体系」方法论的四要素存储与溯源；`checklist_内容是否值得进入知识库` 演示内容准入判断与 `use_when` 调用场景。
